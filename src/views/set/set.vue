                ...
                <div class="header-middle-con">
                    <Row>
                        ...
                        <ul class="top_timeline">
                            <li v-for="(item,index) in progress" :class="{active:index<=childData.progressActiveIndex,current:index==childData.progressActiveIndex}" :key="index">
                ...
                <Modal v-model="addSet" class="modal" title="功能添加" @on-ok="ok" @on-cancel="cancel">
                    <Tabs value="name1">
                        <TabPane label="功能点" name="name1">
                            <span v-for="item in functionList" class="check_a" :class="item.checked?'checked':''" @click="item.checked=!item.checked">
